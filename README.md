...
- A project's items are grouped by kind (pull requests, issues, runs), newest first; `group-by` (`"repository"`, `"date"`, `"author"`, `"none"`), `sort-by` (`"created"`, `"title"`) and `subsections = true` (a subheader over each group) change that, and `show-first = 5` shows each group's first five rows with a Show more row, in `[defaults]` or per project.
...
A project's `repositories` takes single repositories (`owner/name`), everything an owner has (`my-org/*`), and the groups `owned`, `organizations` and `collaborator` (your own repositories, your organizations', and other people's you collaborate on). `repositories = ["anywhere"]` with `pull-requests = { review-requested = true }` lists the pull requests waiting on your review in any repository, even one you've never committed to. Groups and wildcards leave out archived repositories (`archived = true` brings them in) and keep forks (`forks = false` leaves them out), and pick up new repositories within the hour, or at once on ⌘R.
