...
- Attach a screenshot when it shows the bug. `gh` can't upload images to an issue, so screenshots are committed to the repository under `docs/assets/<topic>/` (for example `docs/assets/bugs/`, or the effort's folder such as `docs/assets/shipyard-0.0.2/`) on the working branch, and pushed. Embed the raw URL pinned to the commit that added the file, not a branch name: `![what it shows](https://raw.githubusercontent.com/yahyabedirhan/shipyard/<commit>/docs/assets/<topic>/<name>.png)`. Check it loads (`curl -s -o /dev/null -w "%{http_code}" <url>` prints `200`) before posting. Pull requests here merge with merge commits, so the pinned commit stays reachable from `main` after the merge; don't squash or rebase a branch whose commits are embedded. The orphan branch `issue-assets` is retired: don't add to it.
