...
- **Name**: every effort has a kebab-case name, carried as the label `effort:<name>` on the spec and on every one of its tickets (for example `effort:shipyard-0-0-1` on #1 and its tickets, `effort:sign-in-without-gh` on #22 and its tickets). When publishing a new spec, create its label (`gh label create effort:<name> --color c5def5`; every effort label shares this pale blue, since the name tells them apart) and apply it to the spec and each ticket; a ticket moved to another effort swaps its label too. Refer to an effort by this name, and list it with `gh issue list --state all --label effort:<name>`.
