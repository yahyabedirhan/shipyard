...
- The app never rewrites a file that holds the user's settings, because TOMLDecoder only decodes and a rewrite would lose their comments. It has three writers (ADR 0001 and its amendments); the first two touch only their own lines, and the third writes a whole file only over one that holds nothing of the user's:
  ...
  - `ConfigStore.writePreset(_:projects:)`, onboarding's preset step's (`Shipyard.choosePreset(_:projects:)`), writes `Preset.text(projects:)`, the whole commented file of `my-agents`, `incoming-contributions` or `review-queue` with the repositories picked for it. It writes only when the file is missing or its only live key is `version`, as in `Configuration.header`: `Configuration.acceptsPreset(_:)` in `PresetSetting.swift` reads the file and asks `TOMLSourceMap` for its entries, and comments and blank lines don't count, but any other key or any table, even an empty one, does. A file with anything else, or one that doesn't read, is refused with a `ConfigError` and nothing is written; so are the picker's projects when one is invalid (as for `append`), or when the preset wouldn't read with them (a picked project named like the preset's own). Every reload records the same check in `ConfigStore.acceptsPreset`, and `Shipyard.presets` follows it: all three while a preset may be written, none once the file has settings, when onboarding shows the plain picker instead. `writePreset` checks the file again before writing, so a setting saved since the last reload is never overwritten; `choosePreset` then reloads, and onboarding falls back to the picker. An existing file is written in place, so a symlinked file stays a symlink. The menu follows the reload `writePreset` returns, as after the picker: with projects, the phase moves to `ready` and the first refresh runs, without a restart.
...
6. Cross-key rules the reader enforces and the schema can't express: project names unique, a repository listed once per project ignoring case, and a project using `anywhere` lists pull requests with `review-requested = true` and no issues or runs, once its defaults are merged (rejected on the `anywhere` entry's line).
...
Authors and repositories are named with **selectors** (ADR 0002): a bare word is a group, `@` marks a login, `/` a repository. The key a selector sits under says which set it's from, so groups need no symbol. `Config/Selectors.swift` holds them:
...
6. **The README.** When the setting is something a user meets before reading the skill, give it a line in `README.md`'s "The configuration file" section, and when the README's examples use it, keep them decoding (`ReadmeDocumentTests` checks them). The README stays short; the skill is the reference.
